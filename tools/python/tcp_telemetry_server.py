...
class TelemetryDecoder:
    ...
    def decode(self, payload):
        ...
        return {
            ...
            'wz': data[5],
            'roll': data[6], 'pitch': data[7], 'yaw': data[8],
            'x': data[9], 'y': data[10],
            'vx': data[11], 'vy': data[12],
            'speed_kmh': data[13],
            'mode': self.MODE_NAMES[data[14]] if data[14] < 4 else 'UNKNOWN',
            'lat': data[15], 'lon': data[16],
...
class TelemetryDisplay:
    ...
    def print_full(self, data, rate):
        print("\n" + "="*100)
        print(f"📡 FULL TELEMETRY @ {rate} Hz | {datetime.now().strftime('%H:%M:%S.%f')[:-3]}")
        print("="*100)
        
        print(f"🚗 Motion:     Speed: {data['speed_kmh']:6.1f} km/h  |  Mode: {data['mode']}")
        print(f"               Vel:   vx={data['vx']:+6.2f}  vy={data['vy']:+6.2f} m/s")
        
        print(f"📐 Orientation: Roll:  {data['roll']*57.3:+6.1f}°  Pitch: {data['pitch']*57.3:+6.1f}°  Yaw: {data['yaw']*57.3:+6.1f}°")
        
        print(f"⚡ Accel:       ax={data['ax']:+6.2f}  ay={data['ay']:+6.2f}  az={data['az']:+6.2f} m/s²  |  wz={data['wz']*57.3:+5.1f}°/s")
    ...
    try:
        while True:
            ...
            client_thread = threading.Thread(
                ...
                args=(conn, addr, decoder, display),
                daemon=True
            )
            client_thread.start()

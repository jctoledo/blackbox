...
fn main() {
    ...
    let mut ekf = Ekf::new();
    let mut mode_classifier = ModeClassifier::new();  // Initialize mode classifier
            ...
            if gps_parser.feed_byte(buf[0]) {
                ...
                if gps_parser.last_fix.valid {
                    ...
                    if let Some((vx, vy)) = gps_parser.get_velocity_enu() {
                        ...
                        // Zero velocity update if stationary (improved detection)
...
/// Check if vehicle is stationary (improved with position-based speed)
fn is_stationary(ax: f32, ay: f32, wz: f32, gps_speed: f32, position_speed: f32) -> bool {
    const ACC_THR: f32 = 0.12 * 9.80665; // 0.12 g (raised for walking)
    const WZ_THR: f32 = 10.0 * 0.017453293; // 10 degrees/s in rad/s
    const GPS_SPEED_THR: f32 = 0.5; // m/s
    const POS_SPEED_THR: f32 = 0.2; // m/s from position delta
...
fn publish_telemetry_ekf(
    ...
    let ekf_speed_kmh = mode_classifier.get_speed_kmh();  // Use mode classifier's smoothed speed
    ...
    let payload = json!({
        ...
        "mode": mode_classifier.get_mode().as_str()  // Use actual mode
    }).to_string();

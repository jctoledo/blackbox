...
async function update(){
  try{
    ...
    gpsEl.textContent=d.sensor_rates.gps_hz.toFixed(0)+' / '+d.sensor_rates.gps_expected.toFixed(0)+' Hz';
    ...
    $('bias-x').textContent=d.ekf.bias_x.toFixed(4)+' m/s²';
    $('bias-y').textContent=d.ekf.bias_y.toFixed(4)+' m/s²';
        ...
        server.fn_handler(
            ...
            move |req| -> Result<(), esp_idf_svc::io::EspIOError> {
                let json = if let Some(diag_state) = state_diag_api.diagnostics() {
                    ...
                    format!(
                        concat!(
                            r#"{{"sensor_rates":{{"imu_hz":{:.1},"gps_hz":{:.1},"imu_expected":{:.0},"gps_expected":{:.0}}},"#,

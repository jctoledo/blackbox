...
class TelemetryDecoder:
    ...
    def decode(self, payload):
        ...
        if checksum_calc != checksum_recv:
            self.error_count += 1
            print(f"⚠️  Checksum mismatch: calc=0x{checksum_calc:04X} recv=0x{checksum_recv:04X} (ignoring)")
...
class TelemetryDisplay:
    ...
    def print_full(self, data, rate):
        ...
        print("\n" + "="*80)
        print(f"📡 Telemetry @ {rate} Hz | Time: {datetime.now().strftime('%H:%M:%S.%f')[:-3]}")
        print("="*80)
        ...
        print(f"\n⚡ Acceleration:")
        print(f"  ax: {data['ax']:+6.2f}  ay: {data['ay']:+6.2f}  az: {data['az']:+6.2f} m/s²")
